...
def lab3(data: Dict[str, np.ndarray]):
    ...
    w_f_eq, w0_f_eq = fisher_linear_params(M1_hat, B1_hat, M2_hat, B2_hat)
    ...
    w_f_une, w0_f_une = fisher_linear_params(M1_hat3, B1_hat3, M2_hat3, B2_hat3)
    ...
    bayes_err_eq = 0.5 * (np.mean(bayes_pred_eq_0 == 1) + np.mean(bayes_pred_eq_1 == 0))
    ...
    bayes_err_une = 0.5 * (np.mean(bayes_pred_une_0 == 1) + np.mean(bayes_pred_une_1 == 0))
    ...
    results = {
        'equal_cov': {
            ...
            'fisher_train_err': 0.5 * (np.mean(classify_linear(X1, w_f_eq, w0_f_eq) == 1) + np.mean(classify_linear(X2, w_f_eq, w0_f_eq) == 0)),
            'mse_train_err': 0.5 * (np.mean(classify_linear(X1, w_mse_eq, w0_mse_eq) == 1) + np.mean(classify_linear(X2, w_mse_eq, w0_mse_eq) == 0)),
            'rm_train_err': 0.5 * (np.mean(classify_linear(X1, w_rm_eq, w0_rm_eq) == 1) + np.mean(classify_linear(X2, w_rm_eq, w0_rm_eq) == 0)),
        ...
        'unequal_cov': {
            ...
            'fisher_train_err': 0.5 * (np.mean(classify_linear(X1_3, w_f_une, w0_f_une) == 1) + np.mean(classify_linear(X2_3, w_f_une, w0_f_une) == 0)),
            'mse_train_err': 0.5 * (np.mean(classify_linear(X1_3, w_mse_une, w0_mse_une) == 1) + np.mean(classify_linear(X2_3, w_mse_une, w0_mse_une) == 0)),
            'rm_train_err': 0.5 * (np.mean(classify_linear(X1_3, w_rm_une, w0_rm_une) == 1) + np.mean(classify_linear(X2_3, w_rm_une, w0_rm_une) == 0)),
...
def write_report_text(lab3_res):
    ...
    lines.append(f"Равные ковариации: Bayes={e['bayes_train_err']:.4f}, Fisher={e['fisher_train_err']:.4f}, MSE={e['mse_train_err']:.4f}, RM={e['rm_train_err']:.4f}")
    u = lab3_res['unequal_cov']
    lines.append(f"Неравные ковариации: Bayes={u['bayes_train_err']:.4f}, Fisher={u['fisher_train_err']:.4f}, MSE={u['mse_train_err']:.4f}, RM={u['rm_train_err']:.4f}")
    lines.append('')
